...
import { updatePageAction } from "@/app/(admin)/[adminPath]/(protected)/pages/actions";
import { createPageFormState, type PageFormValues } from "@/lib/admin/page-form";
...
export function PageEditForm({ adminPath, pageId, mediaOptions, initialValues }: PageEditFormProps) {
  const initialState = createPageFormState(initialValues);
  const [state = initialState, formAction, isPending] = useActionState(updatePageAction, initialState);
