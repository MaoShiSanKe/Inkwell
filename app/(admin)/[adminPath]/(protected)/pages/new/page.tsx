...
type AdminPageNewPageProps = {
  params: Promise<{
    adminPath: string;
  }>;
};

export default async function AdminPageNewPage({ params }: AdminPageNewPageProps) {
    ...
    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-6 py-16">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex flex-col gap-3">
          ...
          <h1 className="text-3xl font-semibold tracking-tight">新建页面</h1>
          <p className="text-base leading-7 text-slate-600 dark:text-slate-300">创建站点独立页面，例如关于页或联系页。</p>
        </div>
      ...
      <PageCreateForm adminPath={adminPath} mediaOptions={mediaOptions} />
    </main>
  );
}
