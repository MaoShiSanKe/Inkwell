...
import { listCategoryParentOptions } from "@/lib/admin/taxonomies";
...
export default async function AdminCategoryNewPage({ params }: AdminCategoryNewPageProps) {
  const { adminPath } = await params;
  const parentOptions = await listCategoryParentOptions();
    ...
    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-6 py-16">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex flex-col gap-3">
          ...
          <h1 className="text-3xl font-semibold tracking-tight">新建分类</h1>
          <p className="text-base leading-7 text-slate-600 dark:text-slate-300">
            填写分类名称、slug、描述与父分类，创建最小可用的后台分类项。
          </p>
        </div>
